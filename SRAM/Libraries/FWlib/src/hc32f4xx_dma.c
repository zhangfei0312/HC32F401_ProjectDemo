...
/**
  ...
  * @param  DMAy_Streamx: where y can be 1 to select the DMA and x can be 0 to 7 to select the DMA Stream.
  * @retval None
  */
...
/**
  ...
  * @param  DMAy_Streamx: where y can be 1 to select the DMA and x can be 0
...
void DMA_ClearFlag(DMA_Stream_TypeDef* DMAy_Streamx, uint32_t DMA_FLAG)
{
  ...
  if ((DMA_FLAG & HIGH_ISR_MASK) != (uint32_t)RESET)
  {
    ...
    DMA1->HIFCR = (uint32_t)(DMA_FLAG & RESERVED_MASK);
  }
  else 
  {
    /* Set DMAy LIFCR register clear flag bits */
    DMA1->LIFCR = (uint32_t)(DMA_FLAG & RESERVED_MASK);
  }    
}
...
void DMA_ClearITPendingBit(DMA_Stream_TypeDef* DMAy_Streamx, uint32_t DMA_IT)
{
  ...
  if ((DMA_IT & HIGH_ISR_MASK) != (uint32_t)RESET)
  {
    ...
    DMA1->HIFCR = (uint32_t)(DMA_IT & RESERVED_MASK);
  }
  else 
  {
    /* Set DMAy LIFCR register clear interrupt bits */
    DMA1->LIFCR = (uint32_t)(DMA_IT & RESERVED_MASK);
  }   
}
